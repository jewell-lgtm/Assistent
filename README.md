...
- **Not hardened for the open internet.** Multi-user now, but each user is one bearer token (rotatable) on a pass-through edge with no rate limit yet. The `/api/system/*` surface can rebuild and redeploy the server — it is exposed by deliberate choice because roaming self-modification is the point. You should not run this as-is for anything you care about.
- **Not polished.** The UI is intentionally ugly. The run registry is in-memory (pod restart loses live-run tracking; results survive via git). Android-only, arm64-only APK.
- **Not stable.** Interfaces (the capability contract, the userland kit, persistence) have existed for roughly a day each.

## Architecture in one paragraph

pnpm/turborepo monorepo. `server/` is Effect-TS (`@effect/platform-node`), one container **per user** on OrbStack k8s on a Mac mini (same image; per-user namespace, NodePort, sqlite, userspace checkout + bare git remote, coding-agent creds, OTA dir — provisioned by `scripts/user-add.sh`, routed by per-user subdomain), each serving the API, the OTA protocol, and the embedded coding agent. `app/` is Expo (SDK 57) with all native modules pre-baked so features ship as pure-JS OTA updates; one generic APK pairs to a server at first launch (QR/paste payload → device-stored token + a persisted native OTA-URL override). `packages/capabilities-server` and `packages/capabilities-ui` define the contract userspace code compiles against — server features export typed `HttpApi` routers (the UI derives typed clients from the same schema), app features export tab components composed from a small UI kit; a namespaced KV persistence service and a Pi client are injected as Effect services. `scripts/gen-userspace.mjs` regenerates the feature registry at build, boot, and publish; broken features are isolated (server loader skips them, per-tab ErrorBoundary contains them). A host-side daemon (`opsd`, LAN-only) executes reloads and OTA publishes with per-user tokens (a user's pod can only touch its own stack; core redeploys are admin-only); the pod reaches it by proxy, so the phone can trigger both.
